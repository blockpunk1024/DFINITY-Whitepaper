...
链密钥密码学还包括一系列复杂的技术，用于随着时间的推移稳健、安全地维护基于区块链的复制状态机，这些技术共同构成了我们所说的 **链进化技术** 。每个子网根据 **epochs** 来运行（每个epoch通常在几百个区块的数量级）。使用阈值签名和许多其他技术，链演进技术实现了许多基本的维护活动，这些活动以epochs为基本单位，按一定节奏定期执行：
