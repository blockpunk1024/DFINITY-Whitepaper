...
$$\sigma \leftarrow \prod_j  \sigma ^{\lambda j}_j 
\tag{1}$$
...
$$g^{sj}=\prod_{k=0}^{t-1} A_k^{jk}=g^{a(j)}
\tag{2}$$
...
$$vj:=\prod_i \prod_{k=0}^{t-1} A^{jk}_{i,k}=g^{xj}.$$
